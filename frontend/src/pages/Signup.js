    ...
    <div style={s.page}>
      ...
      <div style={s.right}>
        <div style={s.formBox}>
          ...
          <p style={s.subtitle}>Join AmId as a vendor or client</p>

          <label style={s.label}>Full Name</label>
          <input style={s.input} name="name" placeholder="Enter your full name"
            onChange={handleChange} onFocus={e => e.target.style.borderColor = "#C9A84C"}
          ...
          <input style={s.input} name="password" placeholder="Create a password"
            type="password" onChange={handleChange}
            onFocus={e => e.target.style.borderColor = "#C9A84C"}
            onBlur={e => e.target.style.borderColor = "#2A2A2A"} />

          <label style={s.label}>I am a</label>
          <select style={s.input} name="role" onChange={handleChange}>
            <option value="client">Client — I want to rent dresses</option>
            <option value="vendor">Vendor — I want to list dresses</option>
          </select>

          {message && (
            <div style={message.includes("created") ? s.successMsg : s.errorMsg}>
              {message}
            </div>
          )}
...
const s = {
  ...
  formLogo: {
    width: "70px", height: "70px", objectFit: "contain", marginBottom: "20px",
    display: "block"
  },
  title: {
    fontSize: "28px", color: "#0D0D0D", margin: "0 0 6px", fontWeight: "600", letterSpacing: "1px"
  },
  subtitle: {
    fontSize: "14px", color: "#888", margin: "0 0 28px"
  },
  label: {
